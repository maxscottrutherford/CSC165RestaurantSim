...
public class GameServerUDP extends GameConnectionServer<UUID> 
{
	...
	public void processPacket(Object o, InetAddress senderIP, int senderPort)
	{
		...
		{	// JOIN -- Case where client just joined the server
			...
			{	UUID clientID = UUID.fromString(messageTokens[1]);
				...
				System.out.println("CREATE request received from - " + clientID.toString());
				sendWantsDetailsMessages(clientID);
			}
			
			// DETAILS-FOR --- Case where server receives a details for message
			// Format: dsfr,senderID,recipientID,x,y,z
			if (messageTokens[0].equals("dsfr")) {
				...
				sendDetailsForMessage(recipientID, senderID, pos);
			}
	...
	{	try 
		{	String message = new String("create," + clientID.toString());
			...
			message += "," + position[2];	
			forwardPacketToAll(message, clientID);
			System.out.println("Broadcasting CREATE for - " + clientID.toString());
		} 
		catch (IOException e) 
		{	e.printStackTrace();
	}	}
	...
	{	try 
		{	String message = new String("dsfr," + remoteId.toString());
			...
			message += "," + position[1];
			message += "," + position[2];
			System.out.println("Sending DSFR to " + clientID + " for ghost: " + remoteId);
			System.out.println("Full DSFR message: " + message);
			sendPacket(message, clientID);
		} 
		catch (IOException e) 
		{	e.printStackTrace();
